 function [opt]=M0toPD_resamp4G(opt,outMm,interp)
%
...
[segUnderSamp, segUnderSamp_Xform] = mrAnatResliceSpm(double(seg.data(:,:,:)), inv(seg.qto_xyz), bb, outMm, interp, 0);
...
if isfield(opt,'segfile')
    %T1
    filename=fullfile(opt.outDir,['segresmp_' num2str(outMm(1)) '_' num2str(outMm(2)) '_' num2str(outMm(3)) '.nii.gz']);
    dtiWriteNiftiWrapper(single(T1UnderSamp),T1UnderSamp_Xform,filename);
    
    opt.segfile_Org=opt.segfile;
