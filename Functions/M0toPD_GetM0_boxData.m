...
%
%  ~INPUTS~
%                   opt: The structure of optimization parameters
%                    T1:
%                    M0:
%                    BM:
%                    fb:
...
%                    t1:
%                   BM1:
%               boxSize: The 4D size of the box
%                  skip:
%               Segmask:
%               meanVal:
%                    XX:
...
ZZ(2)=opt.Z(fb(1),fb(2),fb(3))+opt.HboxS(3);
    ...
    %get the location of the box we will work on (in image space (x,y,z))
...
Segmask=zeros(size(seg));
   ...
   k=1;
   for ii=TissueType
       if  length(find(seg==ii))>20 % if there are almost no voxels for a tisue type, we won't use it for regularization
           Segmask(find(seg==ii))=k;
           k=k+1;
       end
   end
   if  length(find(Segmask))<(length(Segmask(:))*0.6) % we need R1 data for regularization. If we don't have it, we won't use this box
       skip=1;
   end
   
end   
   %% if R1 values are wrong, we will skip the voxels
       R1=1./t1;
