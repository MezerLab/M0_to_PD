function [opt, G, PD]=M0toPD_smoothGain_step4(opt,PD,PD_filename,G_filename)
...
T1=readFileNifti(opt.T1file);T1=T1.data;
%Brain mask
...
mask=BM & PD>0 ;
mask=mask & PD>median(PD(mask))/1.5 & PD<median(PD(mask))*1.5 & M0>prctile(M0(mask),5) ;%& T1>0.5 & T1<2.5;
 %& PD<median(PD(mask))/2
...
Gi(~mask)=nan;
Gi(isinf(Gi))=nan;
%mask=mask & Gi>prctile(Gi(mask),2) & Gi<prctile(Gi(mask),98) ;
Gi(~mask)=nan;
...
%% 
confX=abs((1-PDfit./PDx)); confX(isnan(confX))=0; confX(isinf(confX))=0;
confY=abs((1-PDfit./PDy)); confY(isnan(confY))=0; confY(isinf(confY))=0;
confZ=abs((1-PDfit./PDz)); confZ(isnan(confZ))=0; confZ(isinf(confZ))=0;
conf=(confX+confY+confZ)./PDvox;
...
G(isinf(G))=nan;
G(G<0)=nan;
mask=PDfit>0 & BM & PDfit>median(PD(mask))/1.5 & PDfit<median(PD(mask))*1.5 & conf<.1;
G(~mask)=nan;
